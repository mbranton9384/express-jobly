...
describe("applyToJob", function () {
  test("works", async function () {
    ...
    const application = await User.applyToJob("u1", jobId);
  ...
  test("not found if no such user", async function () {
    ...
    const jobId = job.rows[0].id;
    try {
      await User.applyToJob("nope", jobId);
      fail();
    } catch (err) {
      expect(err instanceof NotFoundError).toBeTruthy();
    }
  ...
  test("not found if no such job", async function () {
    try {
      await User.applyToJob("u1", 0); // assuming 0 is an invalid job ID
      fail();
    } catch (err) {
      expect(err instanceof NotFoundError).toBeTruthy();
    }
...
describe("get including jobs", function () {
  ...
  test("works with no job applications", async function () {
    let user = await User.get("u2"); // Assuming u2 has no job applications
    expect(user).toEqual({
      username: "u2",
      firstName: "U2F",
      lastName: "U2L",
